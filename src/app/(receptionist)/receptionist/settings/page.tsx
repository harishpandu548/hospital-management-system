...
const SettingsPage = () => {
  ...
  const role = typeof window !== 'undefined' ? localStorage.getItem('receptionist_role') || '' : '';
  const userName = typeof window !== 'undefined' ? localStorage.getItem('receptionist_userName') || '' : '';
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, maxWidth: 800 }}>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
            <div style={{ width: 52, height: 52, borderRadius: 14, background: 'linear-gradient(135deg, #6366f1, #4f46e5)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 22 }}>
              {userName.charAt(0).toUpperCase() || 'R'}
            </div>
            <div>
              <div style={{ fontWeight: 700, color: '#0f172a', fontSize: 15 }}>{userName || '—'}</div>
              <div style={{ fontSize: 12, color: '#6366f1', fontWeight: 600, marginTop: 2 }}>{role}</div>
            </div>
          </div>
          <div style={{ fontSize: 13, color: '#64748b', background: '#f8fafc', borderRadius: 10, padding: '12px 14px' }}>
            ...
            <div style={{ marginTop: 6 }}><strong style={{ color: '#374151' }}>Portal:</strong> Receptionist</div>
          </div>
        </motion.div>
        ...
        >
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button type="submit" disabled={submitting}
              style={{ padding: '11px', borderRadius: 10, background: 'linear-gradient(135deg, #6366f1, #4f46e5)', color: '#fff', fontWeight: 700, fontSize: 14, border: 'none', cursor: 'pointer', marginTop: 4 }}>
